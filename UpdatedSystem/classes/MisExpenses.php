...
//}
    ...
    public function readOneExpense($expense_id) {
        $query = "SELECT user_id, expense_id, expense_name, handled_by, expense_amount, misc_description, payment_method, date FROM " . $this->table_name . " WHERE expense_id = :expense_id";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':expense_id',$expense_id);
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_ASSOC);
